...
import { endOfYearReset, resetClassActivities, clearClassPoints } from '@/lib/portal/actions/data-tools'
...
type Op = 'endOfYear' | 'resetActivities' | 'clearPoints'

const CAPTION = 'mb-1.5 block text-[11px] font-bold uppercase tracking-[0.8px] text-parch-500'

export function DangerZone({ classes }: Props) {
  const router = useRouter()
  const [typed, setTyped] = useState<Record<Op, string>>({ endOfYear: '', resetActivities: '', clearPoints: '' })
  const [classFor, setClassFor] = useState<Record<'resetActivities' | 'clearPoints', string>>({
